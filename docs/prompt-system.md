...
| **L4** | Skill tool listing + `SkillDiscoveryReminder` | do-harness skills + config ignore (M1-S01 / M2) | Avoid full skill dump; progressive list | **Partial** — policy start in M1; firehose reduce M2 |
...
|---------|----------------------------|
| Agent discovery | `crates/codegen/xai-grok-agent/src/discovery.rs` — project `.grok/agents/`, `~/.grok/agents/` |
...
| Hooks (L6 denials) | `xai-grok-hooks` + project `.grok/hooks/` |
...
Five primary-session roles. Discovery: `do-harness/agents/` → project `.grok/agents/` (see `do-harness/README.md`).  
...
5. Progressive skill surface (L4) policy start: M1-S01; deepen M2.
