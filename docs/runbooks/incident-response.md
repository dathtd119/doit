...
2. **CI Quality gates** — failing smoke `cargo check -p xai-grok-pager-bin` or clippy.
3. **User / agent report** — use `.github/ISSUE_TEMPLATE/bug_report.yml`.

## Immediate steps

1. Capture commit (`git rev-parse HEAD`) and session config (`~/.grok/config.toml` model names only — no secrets).
