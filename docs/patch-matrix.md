...
### Applied — P-BRAND-UI (user-facing Grok → Doit)

Product chrome and in-session identity still said **Grok / Grok Build** after CFG-DOIT path seal. This pin rebrands **user-visible** copy only (welcome, notifications, CLI about, L0 identity, user-guide prose). Does **not** rename crates, `GROK_*` env, or model slugs (`grok-build`).
