...
| Agent discovery merge | Project can shadow built-ins; user/bundled cannot (`discovery.rs`) | Place do proof agents under project `.grok/agents` or linked path |
...
| Hook file discovery | `HookSource::Directory` of `*.json` under `~/.grok/hooks/` or project `.grok/hooks/` | Ship do guided hook as JSON on discovery path |
