...
Grok already supports multiple models via TOML (`~/.grok/config.toml`): many `[model.<name>]`, `[models] default`, api backends. Subagent resolution precedence is documented in crate docs and implemented in `resolve_effective_overrides`:
