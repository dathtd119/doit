...
| **`config.roles.toml`** | **D2 SoT** for `[roles.*]` — tools, disallowed_tools, model, color, default role. Merge into `~/.config/doit/config.toml` |
| `scripts/sync-user-config.sh` | **Config-only** merge → `~/.config/doit/config.toml` (`[roles.*]`, toolset, privacy, ensure `[model.*]` for role pins). **Never copies prompts** |
| `scripts/apply-role-contracts.sh` | Rewrite agent frontmatter + body from TOML + `prompts/roles/` (`--apply` / `--validate`) |
| `scripts/verify-role-contracts.sh` | **VAL-ROLE-TOML-001** seed + body-only prompts + agents match TOML |
| `config.models.yaml` | Multi-model registry + role→model assignment (policy overlay; prefer `[roles.*.model]`) |
...
Five primary-session roles. **Contract SoT** = [`config.roles.toml`](./config.roles.toml)
`[roles.*]` (merge into `~/.config/doit/config.toml`). **Body** = [`prompts/roles/`](./prompts/roles/)
...
|------|----------|------|--------|-------------|
| **intake** | `[roles.intake]` | `prompts/roles/intake.md` | `agents/intake.md` | Clarify intent; no implementation |
| **orchestrator** | `[roles.orchestrator]` | `prompts/roles/orchestrator.md` | `agents/orchestrator.md` | Goal/plan/todo; spawn specialists |
| **explorer** | `[roles.explorer]` | `prompts/roles/explorer.md` | `agents/explorer.md` | Fast scout / maps / citations |
| **worker** | `[roles.worker]` | `prompts/roles/worker.md` | `agents/worker.md` | Implementation + targeted verify |
| **oracle** | `[roles.oracle]` | `prompts/roles/oracle.md` | `agents/oracle.md` | Architecture / hard decisions |

```sh
# Edit tools/model/color in config.roles.toml, then:
bash do-harness/scripts/apply-role-contracts.sh --apply   # agents bridge (+ body from prompts/roles)
...
Model pins: prefer `[roles.<role>].model` in TOML. Legacy
...
| [`prompts/roles/*.md`](./prompts/roles/) | `${role_body}` mission/workflow | Via agents bridge |
