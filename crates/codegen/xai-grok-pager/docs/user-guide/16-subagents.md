...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.doit/agents/` or `~/.config/doit/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.doit/personas/`; applied during subagent resolution |
...
Doit discovers file-based personas from these locations, in priority order:

- `.doit/personas/*.toml` (project)
- `~/.config/doit/personas/*.toml` (user)
...
> **Note:** Doit applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Doit resolves the effective model and reasoning effort in this order, highest priority first:
...
Doit manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
model = "grok-build"
prompt_file = ".doit/prompts/researcher.md"
```
...
# instructions_file = ".doit/personas/concise.md"  # or load from a file
```

Doit also discovers roles from `.doit/roles/*.toml` and personas from `.doit/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

Doit shows running and finished work in side panes on the agent screen:
