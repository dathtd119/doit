...
MCP (Model Context Protocol) servers extend Doit with external tool integrations. They let Doit interact with any service that implements the MCP standard.

---

## What Are MCP Servers?

An MCP server is a process that exposes tools to Doit over a standardized protocol. When you configure an MCP server, its tools become available to the model alongside Doit's built-in tools. The model can discover and call these tools during a session.
...
MCP servers are configured in `~/.config/doit/config.toml` under `[mcp_servers.<name>]` sections.

### stdio Transport (Local Process)

Doit spawns a local process and communicates over stdin/stdout:
...
>
> - env `GROK_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; Doit-native
>   wins if both set; Claude-style name, but we bound by **bytes** not tokens)
> - `config.toml` — user-level (`~/.config/doit/config.toml`) **or repo-level**
>   (`.doit/config.toml` anywhere on the cwd → git-root chain; the deepest
...
> ```
>
> Precedence: requirements.toml > env > repo `.doit/config.toml` >
...
doit mcp list
doit mcp list --json          # Machine-readable output
...
doit mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir

# Add a stdio server with environment variables (-e is repeatable)
doit mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

# Add a remote HTTP server
doit mcp add --transport http sentry https://mcp.sentry.dev/mcp

# Add a remote server with an authentication header (--header is repeatable)
doit mcp add --transport http api https://mcp.example.com/mcp --header "Authorization: Bearer YOUR_TOKEN"

# Add a remote SSE server
doit mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
doit mcp remove github

# Diagnose a server's configuration and connectivity
doit mcp doctor               # Check every configured server
doit mcp doctor github        # Check one server
doit mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `doit mcp add` writes to `~/.config/doit/config.toml` (`--scope user`). Use `--scope project` to write to `.doit/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `doit mcp list` shows servers from both scopes, marking project-scoped ones with `(project)`.

`doit mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.
...
MCP servers can be configured per-project by placing a `.doit/config.toml` in your repository:
...
When a server exposes a native HTTP/SSE endpoint, prefer the `url` form over wrapping it in a stdio proxy such as `npx mcp-remote <url>`. Doit handles HTTP/SSE and OAuth directly, so the native form avoids an extra subprocess per session. It also registers Doit's own OAuth client with the provider.

Doit walks from the current directory up to the git repo root, loading `.doit/config.toml` at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.config/doit/config.toml` | All projects | Lowest |
| `<repo-root>/.doit/config.toml` | This repository | Medium |
| `<cwd>/.doit/config.toml` | Current directory | Highest |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Doit reads most other config sections only from `~/.config/doit/config.toml`.
...
You can enable or disable MCP servers during a session without restarting Doit.
...
Doit loads MCP server configurations from multiple sources for compatibility:

| Source | Format | Location | Configurable |
|--------|--------|----------|-------------|
| `config.toml` | Native Doit config | `~/.config/doit/config.toml`, `.doit/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.config/doit/config.toml` or the corresponding environment variable (`GROK_CURSOR_MCPS_ENABLED`, `GROK_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `doit inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).

---

## MCP OAuth

For MCP servers that require OAuth authentication, Doit handles the credential flow automatically. When an MCP server requests OAuth credentials, Doit opens a browser-based authorization flow and stores the resulting tokens for future use.
...
You must authenticate OAuth-based MCP servers before you can use them. Doit stores the resulting tokens under `~/.config/doit/mcp_credentials.json` as local plaintext with owner-only file permissions (`0600` on Unix). Prefer full-disk encryption on the host. After you edit `config.toml`, press `r` in the `/mcps` modal to refresh the server list.
...
To avoid putting secrets in the config file, reference an environment variable with `${VAR}` (or `${VAR:-default}`). Doit expands string fields in `[mcp_servers.*]` — `url`, `command`, `args`, and the values in `env` and `headers` — at load time:
...
For stdio servers, Doit captures the process's standard error to `~/.config/doit/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.config/doit/logs/mcp/filesystem.stderr.log
```

### Viewing Server Status

Use `doit inspect` to see all loaded MCP servers and their sources:

```bash
doit inspect          # Human-readable
doit inspect --json   # Machine-readable
```
