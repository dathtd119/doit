...
If a plugin includes a `plugin.json` manifest, the manifest can override paths or add metadata; otherwise components load from the convention directories. The manifest is optional: without one, Doit discovers the components above from their standard directories.
...
Doit sets these values and overrides any value you declare for the same key in the hook JSON's `env` map. (Doit also sets the `CLAUDE_PLUGIN_ROOT` and `CLAUDE_PLUGIN_DATA` aliases for compatibility.) See the [Hooks guide](10-hooks.md) for every environment variable passed to hooks.

---

## Plugin locations

Doit discovers plugins from these locations, in priority order:

| Location | Scope | Trust |
|----------|-------|-------|
| `_meta.pluginDirs` (`session/new` / `session/load`) | Session -- loaded for that session only | Trusted automatically |
| `--plugin-dir` (CLI flag, `doit agent`) | Process -- loaded for that agent process only | Trusted automatically |
| `.doit/plugins/` | Project -- shared with the team through version control | Requires trust |
| `~/.config/doit/plugins/` | User -- personal plugins for every project | Trusted automatically |
| `[plugins].paths` (config) | Custom directories you add in `config.toml` | Depends on location |

Doit also reads the `.claude/plugins/` equivalents for compatibility. When two plugins share a name, the higher-priority location wins.

The Agent SDKs load per-session plugins through `GrokOptions.plugins`, which arrives as `_meta.pluginDirs` on `session/new` and `session/load`; because the caller controls the directory, these plugins are always trusted -- their hooks and MCP servers activate without a prompt, and they never persist beyond the session. The `--plugin-dir` flag is the process-wide equivalent for direct CLI use (repeatable: `doit agent --no-leader --plugin-dir A --plugin-dir B stdio`); it applies to dedicated agent processes only and is ignored in leader mode (the shared leader discovers its own plugins).
...
```bash
doit plugin list [--json] [--available]   # List installed plugins (--available requires --json)
doit plugin install <source> --trust      # Git URL, GitHub shorthand (user/repo), or local path
doit plugin uninstall <name> [--confirm] [--keep-data]   # Aliases: rm, remove
doit plugin update [<name>]               # Omit the name to update all plugins
doit plugin enable <name>
doit plugin disable <name>
doit plugin details <name>                # Show the plugin's component inventory
doit plugin validate [<path>]             # Validate plugin.json (default: current directory)
doit plugin tag [<path>] [--push] [--force] [--dry-run]   # Tag a release from the manifest version
```

Run `doit plugin install <source>` without `--trust` and Doit prints the source and warns that installing will activate the plugin's hooks, MCP servers, and skills, then stops without installing. Add `--trust` to install it.
...
```bash
doit plugin marketplace list [--json]
doit plugin marketplace add <url>         # Git URL, GitHub shorthand (user/repo), or local path
doit plugin marketplace remove <url>      # Git URL or local path of a configured source
doit plugin marketplace update [<name>]   # Omit the name to refresh all sources
```

### Example: set up a team marketplace

```bash
doit plugin marketplace add my-org/team-plugins
doit plugin marketplace list
doit plugin install my-org/team-plugins --trust
doit plugin list
doit plugin update
```
...
In an interactive session, these commands open the modal on a specific tab. They take no arguments — manage plugins from the modal or with the `doit plugin` CLI.
...
Configure plugin directories and per-plugin state in `~/.config/doit/config.toml`:
...
List a plugin in `disabled` to discover it but skip loading its components. List a plugin in `enabled` to activate it — plugins are disabled by default unless a CLI override or an explicit config path enables them, so add them here to turn them on. Each entry is either a plain plugin name (as shown by `doit plugin list`) or a full plugin ID in the form `<scope>/<hash>/<name>`.

### Hide the plugins UI

To hide the hooks and plugins UI — the `/hooks` and `/plugins` commands and the scrollback annotations — set this in `~/.config/doit/pager.toml`:
...
Place this file at `~/.config/doit/settings.json` or `~/.claude/settings.json`.
...
Doit trusts plugins from `~/.config/doit/plugins/` automatically. Project plugins in `.doit/plugins/` require explicit trust. To trust a plugin, install it with `--trust`:

```bash
doit plugin install <source> --trust
```
...
Run `doit inspect` to see every discovered plugin and what it provides:

```bash
doit inspect          # Show plugins with their skills, agents, hooks, and MCP servers
doit inspect --json   # Emit machine-readable JSON
```
