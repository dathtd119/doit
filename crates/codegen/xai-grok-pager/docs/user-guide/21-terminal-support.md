...
Doit runs as a full-screen TUI. To draw the interface, it relies on terminal escape sequences for color, clipboard, mouse, and full-screen control. Some terminals, multiplexers, and SSH sessions handle these sequences differently.
...
Doit detects these terminal emulators from environment variables:
...
- Inside tmux, the variables Doit needs to identify the terminal don't reach the pager.
...
**Fix**: Apply the two settings above, then restart Doit.
...
Doit writes to the clipboard through up to three routes, shown in the **Clipboard** section of `/terminal-setup`:

- **native** — Doit always writes to the native OS clipboard first.
- **tmux buffer** — inside tmux, Doit also writes to the tmux paste buffer (`tmux load-buffer`).
- **OSC 52** — Doit emits the OSC 52 escape sequence so the outer terminal updates its clipboard. Doit always emits OSC 52 inside tmux. Outside tmux, it emits OSC 52 on Linux, over SSH, or in a container without a display.

**Linux Wayland**: on compositors that support the data-control protocol (GNOME 48+, KDE, Sway, Hyprland — the **Clipboard** section shows `data-control on`; the line is omitted off Wayland) copies work even if the terminal loses focus mid-copy. On older compositors (GNOME 46/47), keep the terminal focused until the copy toast confirms, and install the `wl-clipboard` package (provides `wl-copy`) for the most reliable route — Doit shows a startup warning when this applies. If data-control misbehaves on your compositor, set `GROK_CLIPBOARD_NO_DATA_CONTROL=1` to stop Doit from speaking that protocol entirely — copies then go through the CLI tools (`wl-copy`/`xclip`).

**Linux X11 selections**: X11 **PRIMARY** and **CLIPBOARD** are separate. Selecting text usually fills PRIMARY; an explicit Copy action fills CLIPBOARD. In Doit:

- An unmodified middle click reads PRIMARY only when `DISPLAY` is non-empty. Pure X11 can fall back to the native arboard reader. XWayland must have `xclip` or `xsel` on `PATH`; Doit deliberately disables the arboard fallback there so it cannot substitute Wayland PRIMARY.
...
**SSH and selected text**: a remote Doit process usually cannot read the local terminal's PRIMARY or CLIPBOARD selection. Use terminal-native `Shift+Insert`, or hold `Shift` while middle-clicking when your terminal uses that gesture to bypass mouse reporting. The terminal then sends the local selection through the PTY instead of asking the remote process to access it.

**Unknown terminals over SSH**: when Doit cannot identify the outer terminal, it sends the copy but reports delivery as unverified. If paste fails, reconnect with `doit wrap <ssh command>` or use `/minimal`.

**Known limitation — Apple Terminal + SSH**:
Apple Terminal ignores OSC 52, so copying from a Doit session over SSH can't reach your local clipboard. Use the workaround below.

**Temporary workaround**: Use `doit wrap ssh` instead of plain `ssh` (for example, `doit wrap ssh user@host`). It runs the command in a local PTY that intercepts OSC 52 sequences, including tmux-wrapped ones, and writes their contents to your local clipboard. The same command wraps anything else whose clipboard can't reach you — for example `doit wrap docker exec -it <container> bash` or `doit wrap kubectl exec -it <pod> -- bash`.

`doit wrap` also protects your local terminal from dirty disconnects: if the wrapped command dies while a remote TUI has mouse reporting, the alternate screen, or similar modes enabled (for example the SSH connection drops mid-session), wrap resets those modes on exit instead of leaving the terminal spraying mouse escape codes.

When Doit starts inside an SSH session that isn't already running under `doit wrap`, a one-time contextual tip above the prompt recommends `doit wrap ssh <host>` (it stops appearing on its own once you launch through wrap). To turn it off, set `ssh_wrap = false` under `[ui.contextual_hints]` in `~/.config/doit/config.toml`, or use `/settings` → **Show contextual hints** → **SSH wrap**.

> **Warning**: `doit wrap` is **experimental** and may misbehave in some setups.
...
This setting is off by default for security reasons. Without it, OSC 52 writes from Doit (or any TUI) will be ignored.
...
**Fix**:
- In Zellij or control mode, Doit intentionally runs inline (no alt screen).
- Set `[terminal] alt_screen = "always"` in `~/.config/doit/pager.toml` to force fullscreen.
...
### Problem: Zellij keybindings interfere with Doit (Ctrl+g, Ctrl+o, etc.)

Zellij intercepts many Ctrl/Alt key combinations before they reach full-screen TUIs like Doit.
...
After this, Zellij starts **locked**. Most keys pass through to Doit. Press `Ctrl+g` to temporarily unlock Zellij when you need its pane/session management.

Zellij recommends this approach for TUI users.

### Problem: `Ctrl+Enter` doesn't interject in WezTerm

**Cause**: WezTerm ships with the Kitty keyboard protocol disabled. Doit relies on it to tell `Ctrl+Enter` (interject) and `Shift+Enter` (send in multiline mode) apart from plain `Enter`. Most other terminals enable the protocol when Doit requests it.

For the same reason, in Apple Terminal, Doit binds `Ctrl+O` to interject.
...
**Verify**: Run `/terminal-setup` inside Doit. While a turn is active, you see the interject hint, and `Ctrl+Enter` interjects.
...
plain digits). Doit therefore never negotiates the protocol for these
...
container): `TERM_PROGRAM` isn't forwarded, so Doit sees an `Unknown`
terminal and skips the protocol for the same reason.

**Fix**: Use **`Alt+Enter`** to insert a newline. xterm.js delivers it
reliably as `ESC`+`CR` regardless of the keyboard protocol, and Doit's
...
If Doit's mouse-driven scrolling stops responding and your terminal falls back to its native scrollbar, mouse reporting is off.
