...
Doit is a terminal-based AI coding assistant (forked from Grok Build). It runs as a TUI (Terminal User Interface) that understands your codebase, executes shell commands, edits files, searches the web, and manages tasks.
...
The PowerShell installer automatically adds `%USERPROFILE%\.doit\bin` to your User PATH. Alternatively, install via [Git for Windows](https://gitforwindows.org/) (Git Bash) or MSYS2 using the bash script above. WSL users get the Linux binary automatically.
...
Start Doit by running:

```bash
doit
```

On first launch, Doit opens your browser to authenticate with grok.com. After you sign in, Doit stores your credentials in `~/.config/doit/auth.json`, where they persist across sessions. Doit refreshes your credentials automatically and prompts you to sign in again when they can no longer be renewed.
...
Once authenticated, Doit presents a full-screen TUI with two main areas:

- **Scrollback** -- the conversation history showing your prompts, Doit's responses, tool calls, file edits, and more.
- **Prompt** -- the input area at the bottom where you type messages.

Type a message and press `Enter` to send it. Doit reads files, runs commands, and edits code as needed. Each tool run streams into the scrollback in real time.
...
By default, Doit asks for permission before executing shell commands or editing files. You can approve individually or toggle always-approve mode:

- Press `Ctrl+O` to toggle always-approve mode
- Use the `--yolo` flag at launch: `doit --yolo`
...
Every conversation is a **session**. Sessions are automatically saved to `~/.config/doit/sessions/` and can be resumed later. Each session tracks the full conversation history, tool calls, file edits, and task state.

- Start a new session: `Ctrl+N` or `/new`
- Resume a previous session: `/resume` in the TUI, or `--resume <ID>` from the CLI
- Continue the most recent session: `doit -c`
...
- **Agent messages** -- Doit's responses with full markdown rendering and syntax highlighting
- **Thinking blocks** -- Doit's reasoning process (collapsible)
...
# Experimental scrollback-native render mode. Sticky: plain `doit` reopens in
...
Run Doit non-interactively for scripting, CI/CD, and automation:
...
```bash
doit -p "Review changes for bugs" --output-format json --yolo | jq -r '.text'
```
...
Add per-project instructions by creating an `AGENTS.md` file in your repository. Doit reads these files and injects their contents as a project-instructions message at the start of the conversation:

```
~/.config/doit/AGENTS.md           # Global rules (apply to all projects)
<repo-root>/AGENTS.md       # Repository-level rules
<cwd>/AGENTS.md             # Directory-level rules (highest priority)
```

Deeper files take precedence. Doit also reads `CLAUDE.md` files for compatibility.
