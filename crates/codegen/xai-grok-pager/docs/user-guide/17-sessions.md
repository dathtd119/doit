...
Doit saves every conversation to disk automatically. Whether you work in the TUI, in headless mode, or over agent stdio, Doit records the exchange as a session. You can resume, rewind, or compact it. This document describes how to manage sessions.
...
Sessions are identified by a unique session ID (a UUIDv7 when Doit generates it; a client may supply its own ID with `-s`) and stored on disk under `~/.config/doit/sessions/`. Set `GROK_HOME` to override the base directory; when it is unset, Doit uses `~/.config/doit`.

---

## Storage Layout

Doit stores each session in its own directory, grouped by working directory. It URL-encodes the working directory to name the group. When the encoded name exceeds 255 bytes, it instead uses a slug plus a hash and records the original path in a `.cwd` file inside the group.

```
~/.config/doit/sessions/<encoded-cwd>/<session-id>/
...
End the session and quit Doit:

```
/quit
```

Alias: `/exit`. To leave the current session but stay in Doit, use `/home` to return to the welcome screen.
...
```bash
doit --resume <session-id>
```

Run `doit --resume` without an ID to resume the most recent session for the current directory.
...
When you run `/rewind` (or press **Esc Esc** within 800ms while idle with an empty prompt and conversation messages), Doit:
...
Doit automatically compacts the conversation when the context window approaches its limit. You will see a notification when auto-compact triggers. The `context_window` setting on your model configuration controls when this threshold is reached.
...
doit -p "Continue where we left off" -r <session-id>

# Continue the most recent session in the current directory
doit -p "What were we doing?" -c
```
...
## The doit sessions Subcommand

List or search sessions from the command line. `doit sessions` requires a subcommand:
...
doit sessions list --limit 50

# Search sessions by keyword (matches titles and prompts)
doit sessions search "rate limit"
```

`doit sessions list` shows sessions for the current working directory, grouped by worktree label. Each row lists the session ID, the creation and update dates, the source status, and the summary. `doit sessions search` combines a local SQLite index with remote results.

---

## Worktree Sessions

When working with subagents or session forks, Doit can create isolated git worktrees per session. Each worktree gets its own copy of the working directory, so file changes in one session do not affect another.
...
Resume a session in a fresh worktree with `doit -w -r <session-id>`.
...
Doit stores the conversation as newline-delimited JSON (JSONL). Each line in `updates.jsonl` is a self-contained ACP session update event. This format supports:
...
The smaller state files -- `summary.json`, `plan.json`, and `signals.json` -- are plain JSON rather than JSONL. JSONL is the source of truth for session content; `doit sessions search` additionally maintains a local SQLite FTS5 index over session titles and prompts for fast keyword search.
