...
- **`doit dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.config/doit/config.toml` under `[keybindings]` like every other shortcut.
...
```
 Doit · Dashboard — 4 agents · 2 awaiting
...
The state icon matches Doit's sibling views (
`tasks_pane`):
...
rebound via `~/.config/doit/config.toml`.
...
process. The plan's Phase 4 ("supervisor / `doit --bg`") would list
