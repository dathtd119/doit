...
Headless mode runs Doit non-interactively from the command line. It accepts a single prompt, executes it with full tool access, and returns the result. Use it to automate tasks, script workflows, build integrations, and parse output programmatically.
...
```bash
doit -p "Your prompt here"
```

Doit processes the prompt, runs any necessary tools, and prints the result to stdout. The process exits when the response is complete.
...
doit -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
doit -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
doit -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
doit -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
doit -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
doit -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny sudo
doit -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Allow all bash commands (auto-approve without prompting)
doit -p "Build the project" --allow "Bash"
```
...
On failure, Doit emits an error object (process exit non-zero). Prompt-level
...
Doit may also emit `max_turns_reached` and `auto_compact_*` events; treat the list as non-exhaustive and switch on `type`.

---

## Session Management in Headless Mode

By default, each `doit -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
doit -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
doit -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
doit -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
doit -p "Remember: the secret number is 42" --output-format json
# Output includes "sessionId": "abc123"

# Resume that exact session
doit -p "What's the secret number?" --resume abc123
```
...
```bash
doit -p "Continue where we left off" -c
```
...
```bash
doit -p "Hello" --output-format json | jq -r '.sessionId'
```
...
doit -p "Generate a README" > README.md

# Parse JSON output with jq
doit -p "List files" --output-format json | jq -r '.text'
```
...
doit -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
doit --prompt-file ./prompt.txt
```
...
```bash
doit -p "Review changes for bugs and security issues." \
...
```bash
doit -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
for file in src/*.js; do
  doit -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
Doit's headless mode can be wrapped as an OpenAI-compatible chat completion API:
...
RESULT=$(doit -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
doit -p "Format all files" --yolo

# Run tests and fix failures
doit -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
doit -p "Run the test suite" --yolo
```
...
- **`doit login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`doit login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `doit -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug doit -p "..." 2> debug.log`.

---

## Project Root Discovery

When Doit starts, it discovers the project root by walking upward from `--cwd`
...
Doit discovers that repository as the project root and scopes its discovery (AGENTS.md, skills, git history) to it, which can make
...
Doit stores data in `~/.config/doit` (override with `GROK_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
### Read-Only `~/.config/doit`

For containers or CI, mount `~/.config/doit` read-only:
...
- Resume: `doit -p "continue" --resume "<id>"` or `doit -p "continue" --continue`
