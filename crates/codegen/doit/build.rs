//! Product binary version stamp: `VERSION_WITH_COMMIT` = `<product-ver> (<short-sha>)`.
//!
//! Version precedence: `DOIT_VERSION` → `GROK_VERSION` → repo-root `VERSION` →
...
fn main() {
    ...
    // Keep clap / xai_grok_version path aligned when this crate is the binary root.
