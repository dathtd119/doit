...
mod max_mcp_output_bytes_tests {
    ...
    fn project_chain_deepest_file_wins() {
        ...
        std::fs::create_dir_all(sub.join(".doit")).unwrap();
        std::fs::create_dir_all(root.join(".doit")).unwrap();
        std::fs::write(
            root.join(".doit/config.toml"),
        ...
        std::fs::write(sub.join(".doit/config.toml"), "[ui]\nvim_mode = true\n").unwrap();
        assert_eq!(super::project_max_mcp_output_bytes(&sub), Some(30_000));

        // No .grok files with the key anywhere → None.
        std::fs::remove_file(root.join(".doit/config.toml")).unwrap();
