...
fn local_pager_binary_path() -> Result<PathBuf> {
    Ok(target_dir()?
        .join("debug")
        .join(format!("doit{}", std::env::consts::EXE_SUFFIX)))
}
...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    cmd.current_dir(workspace_root()?)
        .args(["build", "-p", "doit", "--bin", "doit"])
        .stdin(Stdio::null())
        .envs(xai_tty_utils::pager_env());
    xai_tty_utils::detach_std_command(&mut cmd);
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build doit"))?;

    if !output.status.success() {
        bail!(
            "failed to build doit (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
    ...
    if !binary.exists() {
        bail!(
            "doit build completed but binary missing at {}",
            binary.display()
        );
    }
    Ok(())
...
/// 2. `CARGO_BIN_EXE_doit` (set by `cargo test` when linked against package `doit`)
/// 3. Build locally via `cargo build -p doit` (product composition-root binary)
pub fn pager_binary() -> Result<PathBuf> {
    ...
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_doit") {
        let p = PathBuf::from(path);
        if p.exists() {
            return Ok(p);
        }
